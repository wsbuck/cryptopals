...
)
{
    ...
    unsigned char block[16];
    unsigned char state[16];
    unsigned char decrypted_arr[len + 1];
    unsigned char c;
    int counter = 0;
    size_t rem_len = len;
...
void pkcs7_pad(unsigned char *input, unsigned char *output, int input_len)
{
    int pad_len = (input_len % 16) > 0 ? (16 - (input_len % 16)) : 0;
...
void pkcs7_unpad(unsigned char *input, unsigned char *output, int input_len)
{
    int val_len = input_len;
    if (input[input_len - 1] < 16)
    {
        int pad_len = input[input_len - 1];
        if (check_same(input, input_len - pad_len, input_len - 1))
        {
...
int check_same(unsigned char *input, int start, int end)
{
    char check = input[start];
...
void aes_cbc_main(unsigned char *state, unsigned char *expandedKey, int nbrRounds)
{
    int i = 0;
    unsigned char roundKey[16];
    createRoundKey(expandedKey, roundKey);
...
int aes_cbc_encrypt(
    unsigned char *input, unsigned char *output,
    unsigned char *key, unsigned char *IV, size_t len
)
{
    int nbrRounds = 10;
    int expandedKeySize = (16 * (nbrRounds + 1));
