...
int main(void)
{
    ...
    {
        printf("%2.2x%c", expandedKey[i], ((i+1)%16 == 0) ? '\n' : ' ');;
    }
}
...
void expandKey(unsigned char *expandedKey, unsigned char *key, enum keySize size, size_t expandedKeySize)
{
    ...
    while (currentSize < expandedKeySize)
    {
        ...
        // For 256-bit keys we add an extra sbox to the calc
...
void aes_main(unsigned char *state, unsigned char *expandedKey, int nbrRounds)
{
    ...
    createRoundKey(expandedKey + 16 * nbrRound, roundKey);
    subBytes(state);
    shiftRows(state);
    addRoundKey(state, roundKey);
}
